      ...
      <div class="chatlist" v-for="(item, index) in chatlist" :key="'1'+index">
        <router-link :to="{
          ...
          <div class="chat" @click="gotoinfo(item)">
            ...
            <div class="lastchat" v-for="(lastchat, id) in lastchat" :key="'3'+id"><span
                v-if="lastchat.answerid == item.id"> {{ lastchat.content }} </span>
            </div>
            <div class="chatno" v-for="(chat, chatindex) in chatno" :key="'2'+chatindex"><span
                v-if="chat.senderid== item.id"> {{ chat.total<=99 ? chat.total :'99+' }} </span>
            </div>
          </div>
        </router-link>
      </div>
