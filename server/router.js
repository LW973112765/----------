...
router.get("/getsearch", (req, res) => {
  const page = req.query.page || 1;
  const search = req.query.search;
  const sqlStr = `select * from article where concat (author,content,introduction,title) like '%${search}%'  and status = 0 and is_del = 0`;
  db.query(sqlStr, null, (err, result) => {
    const total = result.length;
    const pagesize = 6;
    const n = (page - 1) * pagesize;
    const sql = `select * from article where concat (author,content,introduction,title) like '%${search}%'  and status = 0 and is_del = 0 order by id desc limit ${pagesize} offset ${n} `;
    db.query(sql, null, (err, result) => {
      // console.log('结果', result.length);
      if (result.length > 0) {
        res.send({
...
router.get("/getcollectarticle", (req, res) => {
  ...
  const sqlStr =
    "select * from article where id in (select article_id from collection where username =?) and status =0 and is_del=0";
  db.query(sqlStr, username, (err, result) => {
